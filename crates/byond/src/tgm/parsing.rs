...
fn path(input: &str) -> IResult<&str, &str> {
    context(
        "path",
        recognize(preceded(tag("/"), separated_list1(tag("/"), identifier))),
    )(input)
}

fn variable(input: &str) -> IResult<&str, Variable> {
    context("variable", assignment(identifier, value))(input)
        .map(|(i, (ident, val))| (i, Variable::new(ident, val)))
}
    ...
    FR: FnMut(&'a str) -> IResult<&'a str, R, E>,
{
    ...
    context("assignment", move |input| {
        parser(input).map(|(i, (l, _, r))| (i, (l, r)))
    })
}
    ...
    E: 'a + ParseError<&'a str> + ContextError<&'a str>,
{
    let mut parser = context(
        ...
        preceded(
            ...
            delimited(
                tag("("),
                separated_list1(tag(","), ws(assignment(key, value))),
                tag(")"),
            ),
        ),
    );
